...
=======
  async ({ history, message, isPro }) => {
    ...
    // Serialize history for the system prompt since 'history' property might not be supported in this SDK version
    const historyText = (history || []).map(m => `${ m.role === 'user' ? 'User' : 'Alice' }: ${ m.text || '[Media content]' } `).join('\n');
    const fullSystemPrompt = `${ studinAiSystemPrompt } \n\nHistorique de la conversation(pour contexte): \n${ historyText } `;
    ...
    if (isVoiceQuery) {
      try {
        ...
        return {
          ...
          toolData: toolCalls as any,
        };
      } catch (e) {
        console.error("Text-to-speech failed:", e);
        // Fallback to text-only response if TTS fails
        return { text: textResponse, toolData: toolCalls as any };
      }
    }
